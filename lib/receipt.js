...
async function write_pdf(ticket, event, user) {
  ...
  const promise = new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      await draw_qrcode(pdf, ticket.checkin_url);
      pdf.restore();
      ...
      pdf.moveDown(0.25);
        ...
        pdf.font("Roboto Light");
      pdf.fontSize(14);
      const amount_total = event.price_cents * ticket.quantity / 100;
      const amount_total_usd = DOLLAR_FORMAT.format(amount_total);
      pdf.text(amount_total_usd);
      pdf.moveUp();
      pdf.translate(108, 0);
      pdf.text(format_date_paid(ticket.created_at));
      pdf.moveUp();
      pdf.translate(172, 0);
      pdf.text(user.email);
      pdf.restore();
      ...
      pdf.font("Roboto Light");
      pdf.fontSize(12);
        pdf.text(`Quantity: ${ticket.quantity}`);
      pdf.moveUp();
      pdf.translate(280, 0);
...
if(require.main === module) {
    ...
    (async () => {
        const event = Event.create({
            start_time: new Date("2025-02-28T19:00:00Z"),
            duration_m: 60 * 3,
            price: 1200,
            venue: "Claremont Country Club",
            venue_address: "53 Country Club Road, Claremont, NH",
            title: "Silent Disco",            
        });
        ...
        const user = new User(3, "josh@joshreyes.com");
        const buf = await write_pdf(ticket, event, user);
        fs.writeFileSync("receipt.pdf", buf);
    })();
}
