...
async function send_confirmation(email_address, event) {
  try {
    ...
    const locals = {...event.to_pojo(), layout: null};
    const html = await template.render(
      "emails/event-confirmation-email",
      locals,
    );
    const text = await template.render(
      "emails/event-confirmation-email-text",
      locals,
    );
    const event_ics = await template.render("emails/event-ical", locals);
