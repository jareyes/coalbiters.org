...
const INSERT_QUERY = "INSERT INTO tickets (stripe_session_id, user_id, event_id, confirmation_code, quantity, date_paid) VALUES (?, ?, ?, ?, ?, ?)";
...
class Ticket {
    ...
    get url() {
        return Ticket.get_url(this.confirmation_code);
    }
    ...
    get pdf_url() {
        ...
        return `${protocol}://${host}${mount}/${this.confirmation_code}.pdf?token=${token}`;
    }
    ...
    static get_url(confirmation_code) {
        const {protocol, host} = ROUTES;
        const mount = ROUTES.mount.tickets;
        const token = Ticket.generate_validation_token(confirmation_code);
        // Mounts start with a slash
